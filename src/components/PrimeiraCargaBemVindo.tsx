...
function CadastroLojaPrimeiraCargaForm({
  ...
  return (
    <div
      ...
      className="mt-0 rounded-xl border border-ibiza-lemon/35 bg-gradient-to-b from-ibiza-lemon/[0.12] to-transparent px-3.5 py-3 ring-1 ring-zinc-200/70 dark:border-ibiza-lemon/30 dark:from-ibiza-lemon/[0.06] dark:to-transparent dark:ring-white/5 lg:sticky lg:top-4"
    >
      <p className="text-center text-[10px] font-extrabold uppercase tracking-[0.2em] text-ibiza-lemon/90">
        Atualizar dados da loja
      </p>
      <p className="mt-1.5 text-center text-[11px] leading-relaxed text-zinc-600 dark:text-zinc-400">
        {downloadConcluido
      ...
      >
        <div>
          ...
          <div
            className="min-h-[2.5rem] rounded-lg border border-zinc-200/90 bg-white px-3 py-2 text-sm font-semibold text-zinc-900 dark:border-white/12 dark:bg-black/35 dark:text-zinc-100"
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-lg border border-zinc-200/90 bg-white px-3 py-2 text-sm text-zinc-900 placeholder:text-zinc-400 focus:border-ibiza-lemon/45 focus:outline-none focus:ring-1 focus:ring-ibiza-lemon/30 dark:border-white/12 dark:bg-black/40 dark:text-zinc-100 dark:placeholder:text-zinc-600"
          />
          {tentouEnviar && !waOk ? (
            <p className="mt-0.5 text-[10px] text-red-600 dark:text-red-400">Informe um telefone válido (mínimo 10 dígitos).</p>
          ) : null}
        </div>
        ...
        <div>
          ...
          {tentouEnviar && !emLojaOk ? (
            <p className="mt-0.5 text-[10px] text-red-600 dark:text-red-400">E-mail da loja inválido.</p>
          ) : null}
        </div>
        ...
        <div>
          ...
          {tentouEnviar && !emCobOk ? (
            <p className="mt-0.5 text-[10px] text-red-600 dark:text-red-400">E-mail de cobrança inválido.</p>
          ) : null}
        </div>

        {!downloadConcluido ? (
          <p className="rounded-lg border border-amber-200/90 bg-amber-50/95 px-3 py-2 text-center text-[11px] leading-relaxed text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/20 dark:text-amber-100/90">
            O botão só fica disponível quando o <strong className="font-semibold text-amber-900 dark:text-amber-50">download das músicas</strong> terminar.
          </p>
        ) : null}
        ...
        <button
          ...
          className={
            downloadConcluido
              ...
              : 'mt-1 w-full cursor-not-allowed rounded-xl border border-zinc-200 bg-zinc-100 py-2.5 text-sm font-semibold text-zinc-500 opacity-70 dark:border-white/10 dark:bg-white/5'
          }
        >
...
export function PrimeiraCargaBemVindo({
  ...
  return (
    <div
      className="w-full max-w-5xl rounded-2xl border border-ibiza-magenta/35 bg-zinc-50/98 px-4 py-5 shadow-[0_20px_50px_rgba(0,0,0,0.1)] ring-1 ring-zinc-200/80 sm:px-5 sm:py-6 lg:max-w-6xl lg:px-6 lg:py-5 dark:bg-zinc-950 dark:shadow-[0_28px_70px_rgba(0,0,0,0.72)] dark:ring-white/10"
      role="dialog"
      aria-modal="true"
      ...
      {erroSinc ? (
        <>
          ...
          <h1
            ...
            className="mt-2 text-center text-2xl font-extrabold tracking-tight text-zinc-900 sm:text-3xl dark:text-zinc-50"
          >
            Bem-vindo
          </h1>
          <p className="mt-4 rounded-xl border border-red-300/90 bg-red-50 px-4 py-3 text-center text-sm text-red-900 dark:border-red-900/60 dark:bg-red-950/35 dark:text-red-200">
            {erroSinc}
          </p>
          <button
            type="button"
          ...
          <button
            type="button"
            onClick={() => onSair()}
            className="mt-3 w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm font-semibold text-zinc-800 shadow-sm transition hover:bg-zinc-50 dark:border-white/15 dark:bg-white/5 dark:text-zinc-200 dark:hover:bg-white/10"
          >
        ...
        <>
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            <div className="h-11 w-11 animate-spin rounded-full border-2 border-zinc-200 border-t-ibiza-lemon border-r-ibiza-magenta border-b-ibiza-purple dark:border-zinc-800" />
            <p className="text-center text-sm leading-relaxed text-zinc-600 dark:text-zinc-300">A abrir o player…</p>
          </div>
        </>
      ) : (
        /**
          ...
          <div className="min-w-0 pr-1">
            ...
            {!downloadConcluido && midiaDownload ? (
              <div className="mt-4 w-full">
                ...
                <div className="mt-2 h-4 w-full overflow-hidden rounded-full border border-zinc-200 bg-zinc-100 shadow-inner dark:border-white/15 dark:bg-black/50">
                  <div
            ...
            <div className="mt-3 grid grid-cols-1 gap-2">
              {WHATSAPP_BOTOES_CONTATO.map((w) => (
                <a
                  ...
                  className="flex items-center justify-center rounded-xl border border-emerald-500/50 bg-emerald-50 px-2 py-2.5 text-center text-[11px] font-semibold text-emerald-900 transition hover:bg-emerald-100 dark:border-emerald-600/60 dark:bg-emerald-700/25 dark:text-emerald-100 dark:hover:bg-emerald-600/35"
                >
                  {w.label}
                </a>
              ))}
