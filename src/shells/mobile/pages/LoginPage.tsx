/**
 * Shell mobile (`/m/login`) — edite só aqui o layout/UX touch.
 * (Hoje espelha o desktop; o código diverge quando precisares.)
 *
 * Tela de login.
 *
...
import { publicAssetUrl } from '@/api/config';
...
export function LoginPage() {
  ...
  useEffect(() => {
    if (clienteIdStore != null && clienteIdStore > 0 && !tokenStore?.token) {
      navigate(path('/selecionar-pdv'), { replace: true });
    }
  }, [clienteIdStore, tokenStore, navigate, path]);
